...
int main()
{
    ...
    comm_buffer[99] = '\0'; // just in case

    int connected; //0 for unconnected, 1 for connected, 2 for access mode
    ...
    /*
    for(;;) {
        // This is the loop that actually does things.
        printf("Enter string: ");
        char line[100];
        fgets(line, 99, stdin);
        line[99] = '\0'; // just in case
   ...
   // f_mount(fs, "", 1);
    ...
    for(;;) {
        comm_status = 0; // Waiting
        //char cmd[100] = "WF+PING!\n"; // Just some test command; add handling later.
        printf("WF+PING!\n");
        while (comm_status == 0) {
            fgets(comm_buffer, 99, stdin); // This should be an interrupt in the future...
            comm_buffer[99] = '\0'; // Just in case

            temp = strcmp(comm_buffer, "ACK\n");
            if (temp == 0) {
                if(VERBOSE) printf("ACK received.\n");
                comm_status = 1; // Acknowledged
            }
            temp = strcmp(comm_buffer, "NACK\n");
            if (temp == 0) {
                if(VERBOSE) printf("NACK received.\n");
                comm_status = 2; // Not acknowledged
            }
